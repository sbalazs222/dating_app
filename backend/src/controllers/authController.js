...
export async function register(req, res, next) {
    ...
    try {
        ...
        await pool.query('INSERT INTO users (username, password_hash, email, full_name, birth_date, gender, bio, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [username, hashedPassword, email, fullname, birthdate, gender, bio, latitude, longitude]);
...
export async function login(req, res, next) {
    ...
    try {
        const [exists] = await pool.query('SELECT id, username, password_hash FROM users WHERE email = ?', [email]);
        if (exists.length !== 1) {
        ...
        const token = generateToken({id: exists[0].id, username: exists[0].username});
